...
async function reservasTest() {
  ...
  async function crear_conDiaFeriado_noCreaReserva() {
    ...
    await Chai.assert.isRejected(reservas.CU_crearReserva.run(reserva));
  }
  ...
  async function crear_conDiaNoferiado_creaReserva() {
    ...
    const reservaCreada = await reservas.CU_crearReserva.run(reserva);
  ...
  async function confirmarReserva_reservaExistente() {
    ...
    const reservaCreada = await reservas.crear(reserva);

    await Chai.assert.isFulfilled(reservas.CU_confirmarReserva.run(reservaCreada.id));
  }

  async function confirmarReserva_reservaNoExistente() {
    await Chai.assert.isRejected(reservas.CU_confirmarReserva.run(100));
  }
  ...
  async function eliminarReserva_reservaExistente() {
    ...
    await Chai.assert.isFulfilled(reservas.CU_eliminarReserva.run(reservaCreada.id));
  }

  async function eliminarReserva_reservaNoExistente() {
    await Chai.assert.isFulfilled(reservas.CU_eliminarReserva.run(100));
  }
  ...
  console.log('--------- Los test de ReservasTest pasaron correctamente ---------');
}
